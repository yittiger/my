...
export default {
  ...
  computed: {
    ...
    fixedOffset() {
      const offset = this.innerRadius + this.itemWidth
  ...
  methods: {
    ...
    calCircle(arr, level, index) {
      ...
      const range = Math.abs(angleRange[0] - angleRange[1])
      const itemAngle = parseFloat(range / length)
      const liWidth = innerRadius + this.itemWidth
      const menuWidth = liWidth * 2
      return {
    ...
    cxcommand(event, item, level, index) {
      ...
      if (item[this.tPropMap.subMenu]) {
        ...
        this.reduceMenu(level.levelNum + 1).then(() => {
          const nextLevel = this.menuMap['level' + (item.levelNum + 1)]
          const subMenu = nextLevel['item_' + item[this.itemKey]]
